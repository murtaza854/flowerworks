import { FormControl, InputLabel, Typography, Input, FormHelperText, Button, FormControlLabel, Checkbox } from '@mui/material';
...
function ColorForm(props) {
    ...
    const [name, setName] = useState({ value: '', error: false, helperText: 'Enter a name Ex. Red' });
    ...
    useEffect(() => {
        (
            async () => {
                if (id) {
                    const response = await fetch(`${api}/color/getById`, {
                        method: 'POST',
                        headers: {
                    ...
                    if (content.data) {
                        const { data } = content;
                        setName({ value: data.name, error: false, helperText: 'Enter a name Ex. red' });
                        setPrice({ value: data.price, error: false, helperText: 'Enter a price Ex. 200' });
                        setCheckBoxes({ active: data.active });
                        setDisabled(false);
                    } else {
                        window.location.href = window.location.href.split('/admin')[0] + '/admin/color';
                    }
                }
            })();
    }, [id]);
    ...
    const handleNameChange = (event) => {
        if (event.target.value.length === 0) {
            ...
            setName({ value: event.target.value, error: true, helperText: 'Color already exists!' });
        } else {
            setName({ value: event.target.value, error: false, helperText: 'Enter a name Ex. Red' });
        }
    }
