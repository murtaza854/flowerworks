...
function FlowerForm(props) {
    ...
    const [name, setName] = useState({ value: '', error: false, helperText: 'Enter a name Ex. Rose' });
    ...
    useEffect(() => {
        (
            async () => {
                if (id) {
                    const response = await fetch(`${api}/flower/getById`, {
                        method: 'POST',
                        headers: {
                    ...
                    if (content.data) {
                        const { data } = content;
                        setName({ value: data.name, error: false, helperText: 'Enter a name Ex. Rose' });
                        setPrice({ value: data.price, error: false, helperText: 'Enter a price Ex. 200' });
                        setCheckBoxes({ active: data.active });
                        setDisabled(false);
                    } else {
                        window.location.href = window.location.href.split('/admin')[0] + '/admin/flower';
                    }
                }
            })();
    }, [id]);
    ...
    const handleNameChange = (event) => {
        if (event.target.value.length === 0) {
            ...
            setName({ value: event.target.value, error: true, helperText: 'Flower already exists!' });
        } else {
            setName({ value: event.target.value, error: false, helperText: 'Enter a name Ex. Rose' });
        }
    }
    ...
    const handleSubmitAdd = async event => {
        ...
        const response = await fetch(`${api}/flower/add`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json'
            },
    ...
    const handleSubmitEdit = async event => {
        event.preventDefault();
        const response = await fetch(`${api}/flower/update`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
