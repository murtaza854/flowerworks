...
function AddonForm(props) {
    ...
    const [name, setName] = useState({ value: '', error: false, helperText: 'Enter a name Ex. Cake' });
    ...
    useEffect(() => {
        (
            async () => {
                if (id) {
                    const response = await fetch(`${api}/addon/getById`, {
                        method: 'POST',
                        headers: {
                    ...
                    if (content.data) {
                        const { data } = content;
                        setName({ value: data.name, error: false, helperText: 'Enter a name Ex. Cake' });
                        ...
                        window.location.href = window.location.href.split('/admin')[0] + '/admin/addon';
                    }
                }
            })();
    }, [id]);
    ...
    const handleNameChange = (event) => {
        if (event.target.value.length === 0) {
            ...
            setName({ value: event.target.value, error: false, helperText: 'Enter a name Ex. Cake' });
        }
    }
    ...
    const handleSubmitAdd = async event => {
        ...
        const response = await fetch(`${api}/addon/add`, {
            method: 'POST',
            headers: {
